...
function check_active_sans_descri()
{

  $con = ConBdPresta();

  $sql = "SELECT * FROM ps_product INNER JOIN ps_product_lang ON ps_product.id_product = ps_product_lang.id_product WHERE description_short = '' AND active = 1 OR description_short IS NULL AND active = 1";
...
function check_active_sans_ean()
{
...
function count_new_order()
{
  ...
  $con = ConBdPresta();
 $sql = "SELECT COUNT(*) FROM ps_orders  WHERE current_state = '2' ";
...
function list_prod_inactive()
{

        $con = ConBdPresta();
        $sql = "SELECT * FROM ps_product WHERE active = 0 GROUP BY id_product DESC "  ;
        foreach($con->query($sql) as $row)
                 {
                 //var_dump($row);
                  $idart = $row['id_product'];
